...
class FilterHelper {
     ...
     */
    public static function configureFilter($filterNode, $persist = true) {
        // Install filters.
        ...
        foreach($subNodes as $subNode) { /* @var $subNode XMLNode */
            switch($subNode->getName()) {
                case 'setting':
                    ...
                    list($name, $value) = self::getFilterSetting($subNode);
                    $settings[$name] = $value;
                    unset($name, $value);
                    break;
                ...
                case 'filter':
                    ...
                    $subFilter = self::configureFilter($subNode, false);
        ...
        if ($similarFilterFactory->getCount() > 0) {
            ...
            foreach($similarFilters as $index => $similarFilter) { /* @var $similarFilter PersistableFilter */
                if (!self::compareFilters($similarFilter, $settings, $subFilters)) unset($similarFilters[$index]);
            }
     ...
     */
    public static function compareFilters($filterA, $filterBSettings, $filterBSubfilters) {
        // Compare settings.
        ...
        if (is_a($filterA, 'CompositeFilter')) {
            ...
            foreach($filterBSubfilters as $filterBSubfilter) { /* @var $filterBSubfilter PersistableFilter */
                ...
                // Recurse.
                if (!self::compareFilters($filterASubfilter, $filterBSubfilterSettings, $filterBSubfilterSubfilters)) return false;
            }
        }
     ...
     */
    public static function getFilterSetting($settingNode) {
        ...
        switch($type) {
            ...
            case 'object':
                ...
                $value = self::readArraySetting($arrayNode);
                break;
     ...
     * @return array
     */
    public static function readArraySetting($arrayNode) {
        $resultArray = array();
        foreach($arrayNode->getChildren() as $elementNode) { /* @var $elementNode XMLNode */
            ...
            if (is_a($subArrayNode, 'XMLNode')) {
                ...
                $value = self::readArraySetting($subArrayNode);
            } else {

 ...
 * * REFACTORED: Wizdam Edition (PHP 8 Constructor, No References, Visibility, Hook Dispatch)
 */

import ('core.Modules.reviewForm.ReviewFormResponse');
...
class ReviewFormResponseDAO extends DAO {
     ...
     */
    public function ReviewFormResponseDAO() {
        trigger_error(
            "Class '" . get_class($this) . "' uses deprecated constructor parent::ReviewFormResponseDAO(). Please refactor to use parent::__construct().",
            E_USER_DEPRECATED
        );
        self::__construct();
    }
    ...
    public function _returnReviewFormResponseFromRow($row) {
        ...
        // WIZDAM FIX: Protocol update call -> dispatch
        HookRegistry::dispatch('ReviewFormResponseDAO::_returnReviewFormResponseFromRow', array(&$reviewFormResponse, &$row));
    ...
    public function insertReviewFormResponse($reviewFormResponse) {
        if (Config::getVar('debug', 'deprecation_warnings')) trigger_error('Deprecated function.');
        $this->insertObject($reviewFormResponse);
    }

    public function updateReviewFormResponse($reviewFormResponse) {
        if (Config::getVar('debug', 'deprecation_warnings')) trigger_error('Deprecated function.');
        $this->updateObject($reviewFormResponse);
    }

    public function deleteReviewFormResponse($reviewFormResponse) {
        if (Config::getVar('debug', 'deprecation_warnings')) trigger_error('Deprecated function.');
        $this->deleteObject($reviewFormResponse);
    }

    public function deleteReviewFormResponseById($reviewId, $reviewFormElementId) {
        if (Config::getVar('debug', 'deprecation_warnings')) trigger_error('Deprecated function.');
        return $this->deleteById($reviewId, $reviewFormElementId);
    }

    public function deleteReviewFormResponseByReviewId($reviewId) {
        if (Config::getVar('debug', 'deprecation_warnings')) trigger_error('Deprecated function.');
        return $this->deleteByReviewId($reviewId);
    }

    public function deleteReviewFormResponseByReviewFormElementId($reviewFormElementId) {
        if (Config::getVar('debug', 'deprecation_warnings')) trigger_error('Deprecated function.');

...
class PolicySet {
     ...
     */
    public function PolicySet($combiningAlgorithm = COMBINING_DENY_OVERRIDES) {
        trigger_error(
            "Class '" . get_class($this) . "' uses deprecated constructor parent::PolicySet(). Please refactor to use parent::__construct().",
            E_USER_DEPRECATED
        );
        self::__construct($combiningAlgorithm);
    }
    ...
    public function addPolicy($policyOrPolicySet, $addToTop = false) {
        ...
        assert($policyOrPolicySet instanceof AuthorizationPolicy || $policyOrPolicySet instanceof PolicySet);
        
        if ($addToTop) {
            array_unshift($this->_policies, $policyOrPolicySet);
        } else {
            // Removed & reference assignment

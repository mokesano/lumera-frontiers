...
class AuthorizationDecisionManager {
    ...
    /** @var array */
    public $_authorizationMessages = array();

    /** @var array authorized objects provided by authorization policies */
    public $_authorizedContext = array();
     ...
     */
    public function AuthorizationDecisionManager() {
        trigger_error(
            "Class '" . get_class($this) . "' uses deprecated constructor parent::AuthorizationDecisionManager(). Please refactor to use parent::__construct().",
            E_USER_DEPRECATED
        );
        self::__construct();
    }
    ...
    public function addPolicy($policyOrPolicySet, $addToTop = false) {
        // Removed & reference (objects pass by reference automatically)
    ...
    public function &getAuthorizedContext() {
        // Wizdam Fix: Reference kept here as it's modified externally (by policies via setAuthorizedContext)
        // Context is an array, so returning by ref allows modification of the original array property.
    ...
    public function decide() {
        ...
        $callOnDeny = null;
        // Pass callOnDeny by reference (explicitly required for advice extraction)
    ...
    public function _decidePolicySet(&$policySet, &$callOnDeny) {
        ...
        foreach($policySet->getPolicies() as $policy) {
            ...
            if ($policy instanceof AuthorizationPolicy && $effect == AUTHORIZATION_DENY
            ...
            if ($effect === $overriddenEffect) {
                ...
                if ($policy instanceof AuthorizationPolicy && $dominantEffect == AUTHORIZATION_DENY

...
/**
 * @file core.Modules.user/CoreUserDAO.inc.php
 *
...
class CoreUserDAO extends DAO {
    ...
    public function getById($userId, $allowDisabled = true) {
        ...
        if ($result->RecordCount() != 0) {
            // [NOTE] Memanggil fungsi factory tanpa &
    ...
    /**
     * Retrieve a user by ID. Deprecated in favor of getById().
     * @param $userId int
     * @param $allowDisabled boolean
     * @return User
     */
    public function getUser($userId, $allowDisabled = true) {
        if (Config::getVar('debug', 'deprecation_warnings')) trigger_error('Deprecated function.');
        return $this->getById($userId, $allowDisabled);
    }
    ...
    /**
     * Retrieve a user by username. Deprecated in favor of getByUsername().
     * @param $username string
     * @param $allowDisabled boolean
     * @return User
     */
    public function getUserByUsername($username, $allowDisabled = true) {
        if (Config::getVar('debug', 'deprecation_warnings')) trigger_error('Deprecated function.');
        return $this->getByUsername($username, $allowDisabled);
    }
    ...
    /**
     * Internal function to return a User object from a row, including user settings data.
     * @param $row array
     * @return User
     */
    public function _returnUserFromRowWithData($row) {
        $user = $this->_returnUserFromRow($row, false);
        $this->getDataObjectSettings('user_settings', 'user_id', $row['user_id'], $user);

        HookRegistry::dispatch('UserDAO::_returnUserFromRowWithData', array(&$user, &$row));
    ...
    public function _returnUserFromRow($row, $callHook = true) {
        $user = new User(); // Ensure User class is loaded
        ...
        $user->setUrl($row['url']);
        $user->setGoogleScholar($row['google_scholar']);
        $user->setSintaId($row['sinta_id']);
        $user->setScopusId($row['scopus_id']);
        $user->setDimensionId($row['dimension_id']);
        $user->setResearcherId($row['researcher_id']);
        ...
        if ($callHook) HookRegistry::dispatch('UserDAO::_returnUserFromRow', array(&$user, &$row));
    ...
    public function insertUser($user) {
        ...
        $this->update(
            sprintf('INSERT INTO users
                (username, password, salutation, first_name, middle_name, initials, last_name, suffix, gender, email, url, google_scholar, sinta_id, scopus_id, dimension_id, researcher_id, phone, fax, mailing_address, billing_address, country, locales, date_last_email, date_registered, date_validated, date_last_login, must_change_password, disabled, disabled_reason, auth_id, auth_str, inline_help)
                VALUES
                (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, %s, %s, %s, %s, ?, ?, ?, ?, ?, ?)',
    ...
    /**
     * Get the list of locale field names for this user.
     * @return array
     */
    ...
    /**
     * Update the locale fields for this user.
     * @param $user User
     */
    ...
    /**
     * Update an existing user. Deprecated in favor of updateObject().
     * @param $user User
     */
    ...
    /**
     * Delete a user. Deprecated in favor of deleteObject().
     * @param $user User
     */
	 ...
	 */
    public function getUsersByField($field = USER_FIELD_NONE, $match = null, $value = null, $allowDisabled = true, $dbResultRange = null, $sortBy = null, $sortDirection = SORT_DIRECTION_ASC) {
        $sql = 'SELECT DISTINCT u.* FROM users u';
        
        // [NOTE] Inisialisasi variabel untuk menghindari undefined variable warning
        $var = false; 
        ...
        // [MODERNISASI] Logic retrieveRange disederhanakan

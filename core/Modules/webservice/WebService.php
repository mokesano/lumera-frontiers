 ...
 * * REFACTORED: Wizdam Edition (PHP 8 Compatibility, No Reference Params, Visibility)
 */
...
class WebService {
     ...
     */
    public function _callPostWebService($webServiceRequest, $usePut = false) {
        ...
        if (is_array($postOptions)) foreach ($postOptions as $paramValue) {
            if (is_string($paramValue) && isset($paramValue[0]) && $paramValue[0] == '@') die('CURL parameters may not begin with @.');
        }
     ...
     * @return string the web service result or null on failure
     */
    public function _callGetWebService($webServiceRequest) {
     ...
     * @return string the web service result or null on failure
     */
    public function _callPostWebServiceAsync($webServiceRequest, $usePut = false) {
        ...
        // Open the socket.
        // WIZDAM FIX: Null coalescing for port
        $fp = fsockopen(
            $urlParts['host'],
            $urlParts['port'] ?? 80,
            $errno, $errstr, 30
        ...
        if (!$fp) {
            ...
            $host = $urlParts['host'] . ':' . ($urlParts['port'] ?? '80');
     ...
     * @param $ch object Reference to a curl handle (passed by value in PHP 8 as object)
     */
    public function _authenticateRequest($ch) {
        $username = $this->_authUsername;
        if (!is_null($username)) {
            $password = $this->_authPassword;
     ...
     */
    public function _buildHeaders($webServiceRequest) {
        $headers = array('Accept: ' . $webServiceRequest->getAccept());
        foreach($webServiceRequest->getHeaders() as $header => $content) {

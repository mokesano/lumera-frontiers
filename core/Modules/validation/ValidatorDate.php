...
class ValidatorDate extends ValidatorRegExp {
     ...
     * Constructor.
     */
    public function __construct($dateFormat = DATE_FORMAT_ISO) {
        // WIZDAM FIX: getRegexp is now explicitly static
        parent::__construct(self::getRegexp($dateFormat));
    }

    /**
     * [SHIM] Backward Compatibility
     */
    public function ValidatorDate($dateFormat = DATE_FORMAT_ISO) {
        trigger_error(
            "Class '" . get_class($this) . "' uses deprecated constructor parent::ValidatorDate(). Please refactor to use parent::__construct().",
            E_USER_DEPRECATED
        );
        self::__construct($dateFormat);
    }
     ...
     * @return boolean
     */
    public function isValid($value, $minScope = VALIDATOR_DATE_SCOPE_YEAR, $maxScope = VALIDATOR_DATE_SCOPE_DAY) {
        ...
        if (isset($dateMatches['month'])) {
            if (($dateMatches['month'] >= 1 && $dateMatches['month'] <= 12) || $maxScope == VALIDATOR_DATE_SCOPE_YEAR ) {
                if (isset($dateMatches['day'])) {
                    return (checkdate((int)$dateMatches['month'], (int)$dateMatches['day'], (int)$dateMatches['year']) && $maxScope == VALIDATOR_DATE_SCOPE_DAY);
                } else {
     ...
     * @return string
     */
    public static function getRegexp($dateFormat = DATE_FORMAT_ISO) {
        switch ($dateFormat) {

...
class MetadataRecord {
    ...
    public function MetadataRecord() {
        trigger_error(
            "Class '" . get_class($this) . "' uses deprecated constructor parent::MetadataRecord(). Please refactor to use parent::__construct().",
            E_USER_DEPRECATED
        );
        self::__construct();
    }
    ...
    public function addDescription($metadataDescription, $replace = true) {
        assert($metadataDescription instanceof MetadataDescription);
        ...
        // FIXME: getMetadataSchema() is implied but not defined in this base class
        ...
        // FIXME: getApplicationEntityIdFromMetadataDescription is implied but not defined
     ...
     * @param $applicationEntityId string consisting of 'assocType:assocId'
     * @return MetadataDescription|null true if the description was found and removed, otherwise false
     * @see MetadataRecord::getApplicationEntityIdFromMetadataDescription()
     */
    ...
    public function setDescriptions($descriptions) {
        ...
        foreach($descriptions as $description) {
            if (!($this->addDescription($description, false))) {
                $this->_descriptions = array();
                return false; // Wizdam Fix: Return false immediately on failure
            }
        }
        return true;
    }

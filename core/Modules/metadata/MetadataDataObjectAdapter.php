...
class MetadataDataObjectAdapter extends PersistableFilter {
    ...
    public function __construct($filterGroup) {
        ...
        // Removed & from getInputType() call
        $inputType = $this->getInputType();
        $outputType = $this->getOutputType();
        
        if ($inputType instanceof MetadataTypeDescription) {
            ...
            assert($outputType instanceof MetadataTypeDescription);
     ...
     * @param $filterGroup FilterGroup
     */
    public function MetadataDataObjectAdapter($filterGroup) {
        trigger_error(
            "Class '" . get_class($this) . "' uses deprecated constructor parent::MetadataDataObjectAdapter(). Please refactor to use parent::__construct().",
            E_USER_DEPRECATED
        );
    ...
    public function process($input) {
        ...
        switch (true) {
            case $input instanceof MetadataDescription:
                ...
                if (is_null($targetDataObject)) {
                    ...
                    // Basic check to ensure correct class type (loose check for class name matching)
                    // Note: is_a allows string class name check, instanceof needs class object/literal

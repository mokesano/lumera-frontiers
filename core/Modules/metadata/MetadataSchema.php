...
class MetadataSchema {
     ...
     */
    public function MetadataSchema($name, $namespace, $classname, $assocTypes) {
        trigger_error(
            "Class '" . get_class($this) . "' uses deprecated constructor parent::MetadataSchema(). Please refactor to use parent::__construct().",
            E_USER_DEPRECATED
        );
        self::__construct($name, $namespace, $classname, $assocTypes);
    }
     ...
     * @param $name string|MetadataProperty the unique name of the property within a meta-data schema (can be a property URI) or the property object itself
     ...
     */
    public function addProperty($name, $allowedTypes = METADATA_PROPERTY_TYPE_STRING,
            $translated = false, $cardinality = METADATA_PROPERTY_CARDINALITY_ONE, $displayName = null, $validationMessage = null, $mandatory = false) {
        
        // Wizdam Fix: Handle direct object passing (used in setProperties loop)
        if ($name instanceof MetadataProperty) {
            $property = $name;

...
class ParaciteRawCitationNlm30CitationSchemaFilter extends Nlm30CitationSchemaFilter {
    ...
    public function __construct(FilterGroup $filterGroup) {
        ...
        $citationModuleSetting = new SetFilterSetting('citationModule',
                ...
                self::getSupportedCitationModules());
     ...
     * @return string
     */
    public function getCitationModule() {
    ...
    public function process($input) {
        ...
        foreach(['spage', 'epage'] as $pageProperty) {
        ...
        switch($metadata['genre']) {
            ...
            default:
                $metadataMapping += ['publication' => 'jtitle'];
                if (isset($metadata['title'])) {
                    if (!isset($metadata['publication'])) {
                        $metadata['publication'] = $metadata['title'];
        ...
        foreach ($metadata as $paraciteElementName => $paraciteValue) {
            if (!empty($paraciteValue)) {
                ...
                if ($openurl10PropertyName !== null && $openurl10Schema->hasProperty($openurl10PropertyName)) {
        ...
        assert($nlm30Description instanceof MetadataDescription);
     ...
     */
    public static function getSupportedCitationModules(): array {
        return [

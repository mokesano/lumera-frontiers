...
class Nlm30CitationDemultiplexerFilter extends Filter {
     ...
     */
    public function Nlm30CitationDemultiplexerFilter() {
     ...
     * @param string $originalRawCitation
     */
    public function setOriginalRawCitation($originalRawCitation) {
     ...
     * @return string
     */
    public function getOriginalRawCitation() {
        return $this->_originalRawCitation;
    }

    /**
     * Set the original citation description
     * @param MetadataDescription $originalDescription
     */
     ...
     * @param Nlm30CitationSchemaCitationOutputFormatFilter $citationOutputFilter
     */
    public function setCitationOutputFilter($citationOutputFilter) {
    ...
    /**
     * @see Filter::process()
     * @param array $input incoming MetadataDescriptions
     * @return Citation
     */
    public function process(&$input) {
        ...
        foreach ($citationOptions as $citationOption) {
            ...
            if (!isset($scoredCitations[$confidenceScore])) {
                $scoredCitations[$confidenceScore] = [];
            }
     ...
     * @return int filter confidence score
     */
    public function _filterConfidenceScore($metadataDescription) {
     ...
     *
     * @param array $scoredCitations
     * @param int $scoreThreshold a number between 0 (=no threshold) and 100
     * @return Citation one citation with the "best" values set
     */
    public function _guessValues($scoredCitations, $scoreThreshold) {
        ...
        $maxScoresByPropertyNameAndValue = [];
            ...
            foreach($citationsForCurrentScore as $citationForCurrentScore) {
                ...
                foreach($statements as $propertyName => $value) {
                    ...
                    if (!isset($valuesByPropertyName[$propertyName])) {
                        $valuesByPropertyName[$propertyName] = [];
                    }
                    if (!isset($maxScoresByPropertyNameAndValue[$propertyName])) {
                        $maxScoresByPropertyNameAndValue[$propertyName] = [];
                    }
        ...
        foreach($valuesByPropertyName as $propertyName => $values) {
            ...
            $statements = [$propertyName => $bestValue];

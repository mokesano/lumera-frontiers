...
class Mods34SchemaSubmissionAdapter extends MetadataDataObjectAdapter {
     ...
     */
    public function Mods34SchemaSubmissionAdapter($filterGroup) {
    ...
    public function injectMetadataIntoDataObject($metadataDescription, $targetDataObject, string $authorClassName = 'Author') {
        ...
        $localizedCoverageGeos = $metadataDescription->getStatementTranslations('subject/geographic');
        ...
        $localizedCoverageChrons = $metadataDescription->getStatementTranslations('subject/temporal');
        ...
        $this->injectUnmappedDataObjectMetadataFields($metadataDescription, $submission);
     ...
     * @param string $authorMarcrelatorRole the marcrelator role to be used for submission authors.
    ...
    public function extractMetadataFromDataObject($submission, $authorMarcrelatorRole = 'aut') {
        ...
        if (strlen((string) $submissionDate) >= 4) {
        ...
        // Record identifier
        $mods34Description->addStatement('recordInfo/recordIdentifier[@source="wizdam"]', $submission->getId());

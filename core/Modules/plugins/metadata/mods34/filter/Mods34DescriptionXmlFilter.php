...
class Mods34DescriptionXmlFilter extends PersistableFilter {
     ...
     */
    public function Mods34DescriptionXmlFilter($filterGroup) {
     ...
     * @param DOMDocument $doc the MODS document node.
     * @param string $elementName the name of the root element of the composite
     * @param MetadataDescription $metadataDescription
     * @return DOMElement
     */
    public function _processCompositeProperty($doc, $elementName, $metadataDescription) {
        // Create the root element.
     ...
     *
     * @param DOMDocument $doc the MODS document node.
     * @param DOMElement $root the root node of the MODS document.
     * @param MetadataDescription $mods34Description
     * @return array a hierarchical array of XMLNode|DOMDocument objects
     * representing the MODS document.
     */
    public function _buildDocumentHierarchy($doc, $root, $mods34Description) {
        // Get the MODS schema.
        $mods34Schema = $mods34Description->getMetadataSchema();
        if ($mods34Schema instanceof Mods34Schema) {
            ...
            assert($mods34Schema instanceof Mods34NameSchema);
            $catalogingLanguage = 'undefined';
        }

        // Initialize the document hierarchy with the root node.
        // We use reference for $root here within the array structure logic
        // although objects are passed by handle, keeping structure consistent with legacy logic.
        $documentHierarchy = [
            '@branch' => $root
        ];
        ...
        foreach ($properties as $propertyName => $property) { /* @var $property MetadataProperty */
            if ($mods34Description->hasStatement($propertyName)) {
                ...
                if (!isset($translations[$topLevelNode])) $translations[$topLevelNode] = [];
        ...
        foreach ($properties as $propertyName => $property) { /* @var $property MetadataProperty */
            if ($mods34Description->hasStatement($propertyName)) {
                ...
                if ($translated) {
                    ...
                    assert($mods34Schema instanceof Mods34Schema);
                ...
                foreach($translatedValueArrays as $isoLanguage => $translatedValueArray) {
                    foreach($translatedValueArray as $translatedValue) {
                        ...
                        // IMPORTANT: We use reference here to traverse and modify the array structure in-place
                        ...
                        foreach($translatedNodes as $nodeDepth => $nodeName) {
                            ...
                            if($nodeDepth == $hierarchyDepth) {
                                ...
                                if (isset($currentNodeList[$nodeName])) {
                                    ...
                                    $currentNodeList[$nodeName]['@leaves'] = [];
                                }

                                if ($translatedValue instanceof MetadataDescription) {
                            ...
                            // IMPORTANT: Reference assignment for array traversal
     ...
     * @param string $nodePath an XPath-like string that describes the node to be created.
     * @param string|null $value the value to be added as a text node (if any)
     * @return DOMElement
     */
    public function _createNode($doc, $nodePath, $value = null) {
        ...
        if (count($elementPlusAttributes) == 2) {
            ...
            foreach($unparsedAttributes as $unparsedAttribute) {
                ...
                [$attributeName, $attributeValue] = explode('=', rtrim($unparsedAttribute, ' '));
    ...
    /**
     * Recursively join the document hierarchy into a single document.
     * @param array $documentHierarchy
     * @return DOMElement the root node
     */
    public function _joinNodes($documentHierarchy) {
